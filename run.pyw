...
STATISTIC_BORDER = 	pygame.image.load(os.path.join('images', 'statistic_border.png'))
STATISTIC_BORDER = 	pygame.transform.scale(STATISTIC_BORDER, (600, 178))
...
board_Volume = 	BTN((57, 165, 168), BTN_CENTER,		250+BTNS_MIN_LINE_SPACE, BTN_WIDTH, BTN_HEIGHT+100, text=f'{MENU_MUSIC.cur_volume}')
...
def menu():
	...
	while True:
		...
		draw_text('Prototype 1.0.1', font, (255, 255, 255), MASTER, 20, 20)		
...
'''
Shop is for bought upgrades like better counter. This right panel in game main loop.
'''
class Shop():	
	...
	# This place and draw structure
	# You can buy this Only onece
	def drawOnRightPanel(self, surface, score):
		# check it's possible to buy
		if not self.bought:
			# if u have less then score point 
		...
		# end placing it in right place /\
		
		if self.bought:
			# not active description, information and cost
			draw_text(self.text, pygame.font.SysFont(None, 25), (255, 255, 60), surface, self.x + 150, self.y + 15)
		else:
			# activated description, information and cost
			draw_text(self.text, pygame.font.SysFont(None, 25), LIGHT_GREEN, surface, self.x + 150, self.y + 15)

	# This place and draw structure
	# You can buy this multiply times
	def drawOnCenterPanel(self, surface, score):
		# check it's possible to buy
		if not self.bought: 
			# if u have less then score point 
			if score < self.score_to_buy:  
		...
		# start placing it in right place \/
		img = pygame.transform.scale(source, (70, 70))		
		surface.blit(img, (self.x, self.y))
		icon_source = pygame.image.load(os.path.join('images', "shop_items", self.icon))
		icon = pygame.transform.scale(icon_source, (20, 20))
		surface.blit(icon_source, (self.x + 7, self.y + 7))
		# end placing it in right place /\
		
		if self.bought:
			# not active description, information and cost
			draw_text(str(self.counter), pygame.font.SysFont(None, 30), (255, 255, 60), surface, self.x + 8, self.y + 8)
			draw_text(f"koszt: {self.score_to_buy}", pygame.font.SysFont(None, 35), (255, 255, 60), surface, self.x - 10, self.y + 75)
		else:
			# activated description, information and cost
			draw_text(str(self.counter), pygame.font.SysFont(None, 30), LIGHT_GREEN, surface, self.x + 8, self.y + 8)
			draw_text(f"koszt: {self.score_to_buy}", pygame.font.SysFont(None, 35), LIGHT_GREEN, surface, self.x - 10, self.y + 75)
	...
	def isBoughtMoreThenOnce(self, pos, clicked, score):			
		if pos[0] > self.x and pos[0] < self.x + 70:
			if pos[1] > self.y and pos[1] < self.y + 70:
				if clicked:
					if self.limit > self.counter and self.score_to_buy <= score:														
						self.counter += 1	
						self.bought = True
						self.score_to_buy += 5
						return True
...
def game(mx, my):
	...
	whiteCigarette = Shop((255, 255, 555), 1250, 200, whiteCigaretteDesc, abi, ubi, bbi, whiteCigarette_icon, 10)
	whiteCigaretteHappened = False

	redCigarette_icon ="redCigarette_icon.png"
	redCigaretteDesc = "Dodaje 0.15 papierosa na sekunde. koszt 200"
	redCigarette = Shop((255, 255, 555), 1250, 350, redCigaretteDesc, abi, ubi, bbi, redCigarette_icon, 200)
	redCigaretteHappened = False

	goldCigarette_icon ="goldCigarette_icon.png"
	goldCigaretteDesc = "Dodaje 0.5 papierosa na sekunde. koszt 1000"
	goldCigarette = Shop((255, 255, 555), 1250, 500, goldCigaretteDesc, abi, ubi, bbi, goldCigarette_icon, 1000)
	goldCigaretteHappened = False

	dynamite_icon ="dynamite_icon.png"
	dynamiteDesc = "Dodaje 1.2 papierosa na sekunde. koszt 2000"
	dynamite = Shop((255, 255, 555), 1250, 650, dynamiteDesc, abi, ubi, bbi, dynamite_icon, 2000)
	dynamiteHappened = False


	# -------------- Shop in the Center -------------- #
	abis = "active_border_item_small.png"
	ubis = "unactive_border_item_small.png"
	bbis = "bought_border_item_small.png"

	lighter1_icon ="lighter1.png"
	lighter1 = Shop((255, 255, 555), 745, 200, " ", abis, ubis, bbis, lighter1_icon, 10)

	lighter2_icon ="lighter2.png"
	lighter2 = Shop((255, 255, 555), 875, 200, " ", abis, ubis, bbis, lighter2_icon, 30)
	...
	while running:
		...
		# ======================= CENTER PANEL ======================= #	
		draw_text("Buffs", pygame.font.SysFont(None, 150), (0,135,138), MASTER, 750, 70)
		lighter1.drawOnCenterPanel(MASTER, game_score)		
		if lighter1.isBoughtMoreThenOnce((mx, my), click, game_score):
			if click:
				point_per_click += 1.0
				game_score -= lighter1.score_to_buy

		lighter2.drawOnCenterPanel(MASTER, game_score)		
		if lighter2.isBoughtMoreThenOnce((mx, my), click, game_score):
			if click:
				point_per_click += 2.0
				game_score -= lighter2.score_to_buy
			# do something smart here 20,03,2021
						

		# ======================= RIGHT PANEL ======================= #			
	    # -- still in progress
		draw_text("SHOP", pygame.font.SysFont(None, 150), (0,135,138), MASTER, 1260, 70)
		whiteCigarette.drawOnRightPanel(MASTER, game_score)
		...
		redCigarette.drawOnRightPanel(MASTER, game_score)
		...
		goldCigarette.drawOnRightPanel(MASTER, game_score)
		...
		dynamite.drawOnRightPanel(MASTER, game_score)
		dynamite.isBought((mx, my), click)
		if dynamite.bought:
			if not dynamiteHappened:
				point_per_sec += 1.2
				game_score -= 2000	
				dynamiteHappened = True

		# ------------- Statistics -------------#
		draw_text("Statistics", pygame.font.SysFont(None, 150), (0,135,138), MASTER, 60, 70)

		draw_text("Score:", pygame.font.SysFont(None, 35), (0,135,138), MASTER, 110, 350) 		
		MASTER.blit(STATISTIC_BORDER, (55, 370))
		draw_text(f" {round(game_score, 1)}", pygame.font.SysFont("IMPACT", 55), (0,165,138), MASTER, 120, 420)

		draw_text("Points per sec:", pygame.font.SysFont(None, 35), (0,135,138), MASTER, 110, 550)
		MASTER.blit(STATISTIC_BORDER, (55, 570))
		draw_text(f" +{round(point_per_sec,3)}", pygame.font.SysFont("IMPACT", 55), (0,165,138), MASTER, 120, 620)

		draw_text("Points per click:", pygame.font.SysFont(None, 35), (0,135,138), MASTER, 110, 750)
		MASTER.blit(STATISTIC_BORDER, (55, 770))
		draw_text(f" +{round(point_per_click, 3)}", pygame.font.SysFont("IMPACT", 55), (0,165,138), MASTER, 120, 820)

		# ended here 21.03.2021
